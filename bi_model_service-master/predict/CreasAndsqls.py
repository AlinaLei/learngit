...
'''
bm_user_clusting_platfrom_='''
create table IF NOT EXISTS bimodels.bm_user_clusting_platfrom (
...
where stid not in (171073,177370,177619,176669,177525,10258,10261) and uid = %s and pay_time <= %s and ( order_status = 3 or order_status = 4 ) 
order by pay_time'''
user_order_sql_o='''select O.uid,O.stid,unix_timestamp(paytime)/24/3600 daytime,paytime ,paymoney realprice, orig_price op, oil_price/inspay_orig_price liters
, cast(DATE_FORMAT(paytime, '%%H')  as unsigned)+cast(DATE_FORMAT(paytime, '%%i')  as unsigned)/60 paytoc
, credit_amt cc, coupon_amt cd,unix_timestamp(paytime)-unix_timestamp(ordertime) durt
, DAYOFWEEK(paytime) weekday
, DAYOFMONTH(paytime) monthday
, DAYOFYEAR(paytime) yd
from dw.wei_inspay_orders O 
left join wei_inspay_finance F 
ON O.inspay_sono=F.inspay_sono
where uid = %s and paytime <= %s and ( order_status = 3 or order_status = 4 ) and O.uid=%s
order by paytime'''
df_pms12_sql = "select mid, numb0, numb1, record0, record1, obligate, update_time from bimodels.bm_predictmodel_status "
makeup_sql = """select uid from( 
 select *,max(real_time) mreal_time from bm_user_predict_record 
 where uid not in (select uid from bm_user_predict_) 
 group by uid) P 
where mreal_time < (select FROM_UNIXTIME(record0-300) from bm_predictmodel_status where mid = 2)"""
CON_rollbacksql = "update bimodels.bm_predictmodel_status set update_time=FROM_UNIXTIME(record0-140),record0=record0-140 where mid =2"